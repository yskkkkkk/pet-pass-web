...
이 가이드는 `pet-pass-web` 프로젝트를 Vercel에 성공적으로 배포하고 외부 DB(Supabase)를 연동하는 절차를 설명합니다.

---

## 1. 사전 준비 (Supabase 설정)

매장 등록 신청 데이터를 저장하기 위해 Supabase 프로젝트 설정이 필요합니다.

1. [Supabase](https://supabase.com/) 계정 생성 및 프로젝트 생성.
2. **SQL Editor**에서 아래 SQL을 실행하여 테이블을 생성합니다:
   ```sql
   ...
   ```
3. **Project Settings > API**에서 `URL`과 `anon public` 키를 복사해둡니다.
...
3. **환경 변수 설정 (Environment Variables)**:
   - 아래 항목들을 입력합니다:
     ...
     - `SUPABASE_ANON_KEY`: 1단계에서 복사한 anon 키
...
3. **매장 등록 제안** 기능을 실행해보고 Supabase의 `pending_stores` 테이블에 데이터가 들어오는지 확인하세요.
...
- [ ] Vercel Dashboard에 4개의 필수 환경 변수가 모두 등록되었는가?
- [ ] Supabase 테이블 구조가 생성되었는가?
...
- 새로운 매장을 정식 승인하려면 `data/stores.json`에 Supabase에 쌓인 데이터를 수동으로 추가하고 GitHub에 푸시하세요. (Vercel이 자동으로 재배포합니다.)
