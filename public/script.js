...
/**
 * 3-Stage Variable Bottom Sheet Logic (Mobile)
 */
const SheetState = {
  MINIMIZED: 'MINIMIZED',
  HALF: 'HALF',
...
let currentSheetState = SheetState.HALF;
...
function initBottomSheet() {
  ...
  const onTouchEnd = () => {
    if (!isDragging) return;
    isDragging = false;
    panel.style.transition = 'height 0.3s cubic-bezier(0.19, 1, 0.22, 1)';
    ...
    if (currentHeight < vh * 0.3) {
      setSheetState(SheetState.MINIMIZED);
  ...
  filterHeader.addEventListener('touchstart', onTouchStart, { passive: true });
  handle.addEventListener('mousedown', onTouchStart);
  filterHeader.addEventListener('mousedown', onTouchStart);
...
function setSheetState(state) {
  ...
  switch (state) {
    case SheetState.MINIMIZED:
      ...
      if (storeList) storeList.style.overflowY = 'hidden';
      break;
    ...
    case SheetState.EXPANDED:
      ...
      if (storeList) storeList.style.overflowY = 'auto'; // Enable scrolling only when expanded
      break;
  }
}
